...
export function SubscriptionsPage() {
  ...
  const [actionError, setActionError] = useState('')
  const [form, setForm] = useState({ status:'active', starts_at:'', ends_at:'', amount:'', currency:'USD', method:'mobile_money', reference:'', note:'', price:'', standard:'', express:'', plan_status:'active' })
  ...
  function recordPayment(s) { setActionError(''); setForm({...form,amount:String(s.plan_price_usd||''),currency:'USD',method:'mobile_money',reference:'',note:''}); setModal({type:'payment',item:s}) }
  ...
  function editPlan(p) { setActionError(''); setForm({...form,price:String(p.price_usd),standard:String(p.standard_fee_cdf),express:String(p.express_fee_cdf),plan_status:p.status}); setModal({type:'plan',item:p}) }
  async function savePlan() { setActionError(''); const {error}=await supabase.rpc('erp_update_subscription_plan',{p_plan_id:modal.item.id,p_price_usd:Number(Number(form.price).toFixed(2)),p_standard_fee_cdf:Math.round(Number(form.standard)),p_express_fee_cdf:Math.round(Number(form.express)),p_status:form.plan_status}); if(error)return setActionError(adminUserError(error)); setModal(null); reload() }
  return <>
    <SectionHead eyebrow="Finances" title="Abonnements One Market" desc="Plans, clients, renouvellements et paiements réels."/>
    {(error || actionError) && <div className="alert bad">{error || actionError}</div>}
    <div className="metrics-grid"><Metric icon={Zap} label="Actifs" value={m.active||0}/><Metric icon={Clock3} label="Expirent sous 7 jours" value={m.expiring_soon||0}/><Metric icon={CreditCard} label="Revenu du mois" value={usd(m.revenue_month_usd)}/><Metric icon={Banknote} label="Revenu total" value={usd(m.revenue_total_usd)}/></div>
    <div className="plan-grid">{(data.plans||[]).map(plan => <article className="plan-card" key={plan.id}><span>{plan.duration_months} mois</span><h3>{plan.name}</h3><strong>{usd(plan.price_usd)}</strong><small>Standard {cdf(plan.standard_fee_cdf)} · Express {cdf(plan.express_fee_cdf)}</small><Badge value={plan.status}/>{canManage&&<button className="btn ghost" onClick={() => editPlan(plan)}>Modifier</button>}</article>)}</div>
    <section className="panel"><h3>Abonnements clients</h3><Table headers={['Client','Plan','Statut','Début','Fin','Dernier paiement','']} rows={(data.subscriptions||[]).map(s => [<div><strong>{s.customer_name||'Sans nom'}</strong><span>{s.customer_email||s.customer_id}</span></div>,<div><strong>{s.plan_name}</strong><span>{usd(s.plan_price_usd)}</span></div>,<Badge value={s.status}/>,dateTime(s.starts_at),dateTime(s.ends_at),s.last_payment?<div><strong>{money(s.last_payment.amount,s.last_payment.currency)}</strong><span>{s.last_payment.payment_status}</span></div>:'—',canManage?<div className="button-row compact"><button className="row-action" onClick={() => manageSub(s)}>Gérer</button><button className="row-action" onClick={() => recordPayment(s)}>Paiement</button></div>:'—'])}/></section>
