...
export function FinancePage() {
  ...
  const { data, loading, error, reload } = useLoad(async () => {
    const [{ data: dashboard, error: dashError }, { data: orders, error: orderError }, { data: payouts, error: payoutError }, { data: stores, error: storeError }] = await Promise.all([
      ...
      supabase.from('orders').select('id,order_number,status,payment_method,payment_status,items_total,delivery_fee_cdf,created_at').order('created_at', { ascending: false }).limit(200),
  ...
  return <>
    ...
    {tab === 'orders' && <Table headers={['Commande','Date','Méthode','Produits','Livraison','Paiement','']} rows={data.orders.map(order => [order.order_number, dateTime(order.created_at), order.payment_method === 'mobile_money' ? 'Mobile Money' : 'À la livraison', usd(order.items_total), cdf(order.delivery_fee_cdf), <Badge value={order.payment_status}/>, canManage ? <button className="row-action" onClick={() => openPayment(order)}>Gérer</button> : '—'])}/>} 
...
export function SubscriptionsPage() {
  ...
  async function savePlan() { setActionError(''); const {error}=await supabase.rpc('erp_update_subscription_plan',{p_plan_id:modal.item.id,p_price_cdf:Math.round(Number(form.price)),p_standard_fee_cdf:Math.round(Number(form.standard)),p_express_fee_cdf:Math.round(Number(form.express)),p_status:form.plan_status}); if(error)return setActionError(error.message); setModal(null); reload() }
  return <>
