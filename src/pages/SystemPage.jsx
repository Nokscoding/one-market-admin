  ...
  return <>
    ...
    {loading||!payments||!commerce?<Loader/>:<>
      <div className="detail-grid">
        <section className="panel settings-form"><h3>Marketplace & paiements</h3><p className="settings-help">Mode V1 : paiement à la livraison uniquement. Mobile Money est affiché « bientôt » sur le site client.</p><label className="checkbox"><input type="checkbox" checked={payments.cod_enabled} onChange={e=>setPayments({...payments,cod_enabled:e.target.checked})}/><span>Paiement à la livraison actif</span></label><label className="checkbox"><input type="checkbox" checked={payments.mobile_money_enabled} disabled/><span>Mobile Money — bientôt</span></label><div className="form-grid"><label>WhatsApp réservé<input value={payments.mobile_money_whatsapp} onChange={e=>setPayments({...payments,mobile_money_whatsapp:e.target.value})}/></label><label>Numéro Mobile Money réservé<input value={payments.mobile_money_display} onChange={e=>setPayments({...payments,mobile_money_display:e.target.value})}/></label></div><button className="btn primary" disabled={saving==='payments'} onClick={()=>saveSetting('payments',{...payments,mobile_money_enabled:false,mobile_money_coming_soon:true})}>{saving==='payments'?'Enregistrement…':'Enregistrer'}</button></section>
      ...
      <section className="panel"><h3>Livraison</h3><p className="settings-help">La livraison est facturée au client, gratuite pour le vendeur et entièrement gérée par One Market.</p>{data.methods.map(method=><div className="line-item" key={method.code}><span><strong>{method.label}</strong><small>{method.description} · {method.is_active?'Actif':'Inactif'}</small></span><div className="button-row compact"><strong>{cdf(method.fee_cdf)}</strong><button className="btn ghost" onClick={()=>openDelivery(method)}>Modifier</button></div></div>)}</section>
    </>}
