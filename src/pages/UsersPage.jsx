...
import { dateTime, ORDER_LABELS } from '../lib/format'
...
export function UsersPage() {
  ...
  return <>
    <SectionHead eyebrow="Marketplace" title="Utilisateurs" desc="Comptes clients, vendeurs, livreurs et personnel One Market." actions={staff?.staff_role === 'SUPER_ADMIN' && <button className="btn primary" type="button" onClick={() => setCreateOpen(true)}>Créer un utilisateur</button>}/>
    <SearchBar value={search} onChange={setSearch} placeholder="Nom, email, téléphone ou UUID"/>
    {loading ? <Loader/> : <Table headers={['Utilisateur', 'Rôle', 'Statut', 'Staff', 'Création', '']} rows={(data || []).map(user => [
      <div><strong>{user.full_name || 'Sans nom'}</strong><span>{user.email}</span></div>,
      user.role,
      <Badge value={user.account_status}/>,
      user.staff_role ? ROLE_LABELS[user.staff_role] : '—',
...
export function UserDetailPage() {
  ...
  const { data, loading, reload } = useLoad(async () => {
    ...
    const [orders, tickets, stores] = await Promise.all([
      supabase.from('orders').select('id,order_number,status,created_at,grand_total,currency').eq('customer_id', id).order('created_at', { ascending: false }).limit(20),
  ...
  async function changeStatus(status) {
    ...
    if (!error) { setPendingStatus(null); setReason(''); reload() }
  }
  ...
  return <>
    ...
    <div className="detail-grid">
      <section className="panel detail-card">
        <h3>Compte</h3>
        <Info label="Rôle" value={user.role}/><Info label="Statut" value={<Badge value={user.account_status}/>}/><Info label="Téléphone" value={user.phone || '—'}/><Info label="Inscrit" value={dateTime(user.created_at)}/><Info label="Accès ERP" value={user.staff_role ? ROLE_LABELS[user.staff_role] : 'Aucun'}/>
        {canSuspend && <div className="button-row">{user.account_status === 'active' ? <button className="btn danger" type="button" onClick={() => setPendingStatus('suspended')}>Suspendre</button> : <button className="btn primary" type="button" onClick={() => { setReason('Réactivation ERP'); setPendingStatus('active') }}>Réactiver</button>}</div>}
      </section>
    ...
    <section className="panel"><h3>Commandes</h3>{data.orders.length ? data.orders.map(order => <NavLink className="line-item" to={`/orders/${order.id}`} key={order.id}><span><strong>{order.order_number}</strong><small>{dateTime(order.created_at)}</small></span><Badge value={order.status} label={ORDER_LABELS[order.status]}/></NavLink>) : <Empty/>}</section>
