...
import { cdf, dateTime, LOGISTICS_LABELS, ORDER_LABELS, PAYMENT_LABELS, usd } from '../lib/format'
...
export function OrdersPage() {
  ...
  return <>
    ...
    <div className="filter-row">{ORDER_FILTERS.map(item => <button type="button" className={status === item ? 'active' : ''} onClick={() => setStatus(item)} key={item}>{item === 'all' ? 'Toutes' : (ORDER_LABELS[item] || 'Autre')}</button>)}</div>
    ...
    {loading ? <Loader/> : <Table headers={['Commande','Client','Date','Statut','Livraison','Paiement','Produits','']} rows={visible.map(order => [
      ...
      <Badge value={order.status} label={ORDER_LABELS[order.status] || 'En cours'}/>,
      `${order.delivery_method === 'express' ? 'Express' : 'Normale'} · ${cdf(order.delivery_fee_cdf)}`,
      <div><strong>{order.payment_method === 'mobile_money' ? 'Mobile Money' : 'Paiement à la livraison'}</strong><span>{paymentLabel(order)}</span></div>,
...
export function OrderDetailPage() {
  ...
  async function savePayment() {
    ...
    if (error) {
      logAdminError('payment-status', error)
      return setActionError(adminUserError(error, 'Impossible de modifier le statut du paiement.'))
    }
  ...
  return <>
    ...
    <section className="panel"><h3>Sous-commandes</h3>{data.sellerOrders.length ? data.sellerOrders.map(sellerOrder => <div className="line-item" key={sellerOrder.id}><span><strong>{sellerOrder.seller_order_number}</strong><small>{sellerOrder.delivery_method === 'express' ? 'Express' : 'Normale'} · livraison One Market · vendeur 0 FC · commission {Number(sellerOrder.commission_percent||0).toFixed(2)}%</small></span><div><Badge value={sellerOrder.status} label={ORDER_LABELS[sellerOrder.status] || 'En cours'}/><small>{usd(sellerOrder.seller_net_amount)} vendeur</small></div></div>) : <Empty/>}</section>
    ...
    <section className="panel"><h3>Historique</h3>{data.events.length ? data.events.map(event => <div className="timeline-item" key={event.id}><Clock3 size={15}/><span><strong>{event.label || ORDER_LABELS[event.status] || 'Mise à jour de la commande'}</strong><small>{dateTime(event.created_at)}</small></span></div>) : <Empty/>}</section>
    ...
    <ConfirmModal open={paymentOpen} title="Modifier le paiement" text={order.order_number} onClose={() => setPaymentOpen(false)} onConfirm={savePayment}><label>Statut<select value={paymentStatus} onChange={e => setPaymentStatus(e.target.value)}>{allowedPayments.map(status => <option key={status} value={status}>{PAYMENT_LABELS[status] || 'Autre statut'}</option>)}</select></label></ConfirmModal>
  </>
}
