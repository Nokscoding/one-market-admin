...
export function HomePromotionsSettings() {
  ...
  return <>
    <section className="panel promo-admin">
      ...
      <div className="promo-admin-controls"><label className="promo-switch"><input type="checkbox" disabled={busy || editorOpen} checked={config.enabled} onChange={e => persist({ ...config, enabled: e.target.checked }, 'Visibilité du carrousel enregistrée.')}/><span>Afficher le carrousel</span></label><label>Rotation automatique<select disabled={busy || editorOpen} value={config.autoplay_seconds} onChange={e => persist({ ...config, autoplay_seconds: Number(e.target.value) }, 'Durée de rotation enregistrée.')}>{Array.from({ length: 13 }, (_, i) => i + 3).map(seconds => <option key={seconds} value={seconds}>{seconds} secondes</option>)}</select></label><label className="promo-size-label">Hauteur ordinateur<div className="promo-size-control"><input type="range" min="180" max="640" step="10" disabled={busy || editorOpen} value={config.desktop_height} onChange={e => setConfig(current => ({ ...current, desktop_height: clampBannerHeight(e.target.value, 180, 640, 320) }))}/><output>{config.desktop_height}px</output></div></label><label className="promo-size-label">Hauteur mobile<div className="promo-size-control"><input type="range" min="140" max="520" step="10" disabled={busy || editorOpen} value={config.mobile_height} onChange={e => setConfig(current => ({ ...current, mobile_height: clampBannerHeight(e.target.value, 140, 520, 240) }))}/><output>{config.mobile_height}px</output></div></label><label>Remplissage du média<select disabled={busy || editorOpen} value={config.media_fit} onChange={e => setConfig(current => ({ ...current, media_fit: e.target.value === 'contain' ? 'contain' : 'cover' }))}><option value="cover">Remplir / recadrer</option><option value="contain">Afficher en entier</option></select></label><button type="button" className="btn ghost promo-layout-save" disabled={busy || editorOpen} onClick={() => persist({ ...config, desktop_height: clampBannerHeight(config.desktop_height, 180, 640, 320), mobile_height: clampBannerHeight(config.mobile_height, 140, 520, 240), media_fit: config.media_fit === 'contain' ? 'contain' : 'cover' }, 'Format de la bannière enregistré.')}>Enregistrer le format</button></div>
      ...
      {editorOpen && <form className="promo-editor" onSubmit={saveDraft} ref={editor}>
        ...
        <div className="promo-editor-workspace">
          ...
          <div className="wide promo-crop-editor">
            <div><strong>Recadrage du média</strong><small>Choisissez la zone visible dans la bannière sans modifier le fichier original.</small></div>
            <label>Horizontal<div className="promo-crop-control"><input type="range" min="0" max="100" step="1" value={draft.crop_x ?? 50} onChange={e => setDraft({ ...draft, crop_x: Number(e.target.value) })}/><output>{Math.round(Number(draft.crop_x ?? 50))}%</output></div></label>
            <label>Vertical<div className="promo-crop-control"><input type="range" min="0" max="100" step="1" value={draft.crop_y ?? 50} onChange={e => setDraft({ ...draft, crop_y: Number(e.target.value) })}/><output>{Math.round(Number(draft.crop_y ?? 50))}%</output></div></label>
            <label>Zoom<div className="promo-crop-control"><input type="range" min="100" max="220" step="5" value={draft.crop_zoom ?? 100} onChange={e => setDraft({ ...draft, crop_zoom: Number(e.target.value) })}/><output>{Math.round(Number(draft.crop_zoom ?? 100))}%</output></div></label>
          </div>
